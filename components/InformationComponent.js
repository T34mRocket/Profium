...
const InformationComponent = ({ toggleInformation }) => {
  ...
  return (
    <Card.Content>
        ...
        <Paragraph>By clicking main categories on top of this screen, you can search pictures from our database. All the selected/clicked filters will be added to a list that will pop up above the main categories. These selected filters can be deleted when no longer needed. See picture below.</Paragraph>
        ...
        <Paragraph>Filters can be also combined for AND type queries (default is OR) by long pressing that selected filter and then dragging it on top of some other filter.</Paragraph>
