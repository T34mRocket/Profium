...
class SortableFlatList extends Component {
  ...
  getSpacerIndex = (move, activeRow) => {
    ...
    // check that the draggable item is on top of another item to combine them
    if(isAfterActive && (spacerIndex !== activeRow)){
        let index = spacerIndex 
        if(index !== undefined){
          if(index === data.length) index = index - 1
          const nextComponentStartX = this._measurements[index].x
          const nextComponentEndX = this._measurements[index].width+this._measurements[index].x
          if((this._move > nextComponentStartX) && (this._move < nextComponentEndX)){
            this.setState({
              onTopOfNextComponent: true
            })
          } else {
            this.setState({
              onTopOfNextComponent: false
  ...
  renderHoverComponent = () => {
    ...
    return !!hoverComponent && (
      <Animated.View style={[
        ...
        onTopOfNextComponent ? styles.onTopOfNextComponent : styles.notOnTop,
    ...
    return (
      <View ref={setRef(index)} collapsable={false} style={{ opacity: 1, flexDirection: horizontal ? 'row' : 'column' }}>
        ...
        <View style={[
          ...
          { opacity: isActiveRow ? 0 : 1, overflow: 'hidden' }, styles.component
        ]}>
          {component}
        </View>
